<template>
  <div class="flex flex-col h-full">
    <PageHeader :title="t`Chart of Accounts`">
      <Button v-if="!isAllExpanded" variant="secondary" @click="expand">{{
        t`Expand`
      }}</Button>
    ...
    >
      ...
      <template v-for="account in allAccounts" :key="account.name">
        <!-- Account List Item -->
        <div
          class="py-2 cursor-pointer hover:bg-accent group flex items-center border-b border-border flex-shrink-0 pe-4"
          :class="[
            account.level !== 0 ? 'text-base' : 'text-lg',
            isQuickEditOpen(account) ? 'bg-muted' : '',
          ]"
        ...
        >
          ...
          <div class="flex items-baseline">
            ...
            <div class="ms-6 hidden group-hover:block">
              <button
                ...
                class="text-xs text-foreground hover:text-foreground focus:outline-none"
              ...
              <button
                v-if="account.isGroup"
                class="ms-3 text-xs text-foreground hover:text-foreground focus:outline-none"
          ...
          <p v-if="!account.isGroup" class="ms-auto text-base text-foreground">
        ...
        <div
          ...
          class="px-4 border-b border-border cursor-pointer hover:bg-accent group flex items-center text-base"
        ...
        >
          ...
          <div class="flex ms-4 h-row-mid items-center">
            <input
              ...
              class="focus:outline-none bg-transparent placeholder:text-muted-foreground"
              :class="{ 'text-muted-foreground': insertingAccount }"
            ...
            <button
              ...
              class="ms-4 text-xs text-foreground hover:text-foreground focus:outline-none"
              @click="
                (e) => createNewAccount(account, account.addingGroupAccount)
              "
            >
...
export default defineComponent({
  ...
  methods: {
    ...
    async setTotalDebitAndCredit() {
      const totals: TotalCreditAndDebit[] = await getTotalCreditAndDebit(
        this.fyo
      );
