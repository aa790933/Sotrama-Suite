<template>
  <div
    class="relative bg-card border border-border flex-center overflow-hidden group"
    :class="{
      rounded: size === 'form',
  ...
  >
    ...
    <div v-else :class="[!isReadOnly ? 'group-hover:opacity-90' : '']">
      <div
        ...
        class="flex h-full items-center justify-center text-muted-foreground font-semibold w-full text-4xl select-none"
      >
        {{ letterPlaceholder }}
      </div>
      <svg
    ...
    >
      <button class="bg-muted p-0.5 rounded mb-1" @click="handleClick">
        <FeatherIcon
