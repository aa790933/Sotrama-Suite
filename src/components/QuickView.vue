<template>
  <div style="min-width: 192px; max-width: 300px">
    ...
    >
      ...
      <p class="font-semibold text-base text-muted-foreground">
        {{ schema?.label ?? '' }}
      </p>
    </div>
    <div v-if="values.length" class="flex gap-2 p-2 flex-wrap">
      <p v-for="v of values" :key="v.label" class="pill bg-muted">
        <span class="text-muted-foreground">{{ v.label }}</span>
        <span class="text-foreground ml-1.5">{{ v.value }}</span>
      </p>
    </div>
  </div>
</template>
