...
import {
  detectLanIp,
  pingMariaDB as pingMariaDBHost,
  provision as provisionHost,
} from '../provision/hostProvision';
...
export interface AppOps {
  getEnv(
    ...
    version: string
  ): Promise<{ isDevelopment: boolean; platform: string; version: string }>;
  getLanguageMap(
    code: string
  ): Promise<{ languageMap: unknown; success: boolean; message: string }>;
  ...
  sendAPIRequest(
    endpoint: string,
    options: NodeFetchRequestInit | undefined,
    event: Electron.IpcMainInvokeEvent
  ): Promise<unknown>;
...
async function getErrorHandledResponse<T>(
  fn: () => Promise<T> | T
): Promise<BackendResponse> {
  try {
    const data = await fn();
...
export class IpcRouter {
  ...
  constructor(private readonly deps: IpcRouterDeps) {
    this.senderPolicy =
      deps.senderPolicy ??
      new ElectronSenderPolicy(() => deps.windowProvider.getWindow());
    ...
    this.connectionStore =
      deps.connectionStore ?? new ElectronStoreConnectionStore();
    ...
    this.dbOps = {
      ...
      upsertConnection: this.upsertConnection.bind(this),
    };
  ...
  private assertValidSender(event: Electron.IpcMainInvokeEvent): void {
    if (!this.senderPolicy.isValidSender(event))
  ...
  /**
   * Main-process-owned connection persistence. The renderer must never write
   * `connections` (password custody); it calls this instead of
   * `config.set('connections', …)`, which the preload guard rejects.
   */
  async upsertConnection(payload: {
    ...
    try {
      let id = payload.connectionId;
      let persisted = false;
      if (payload.config) {
        sanitizeDatabaseName(payload.config.database);
        id = this.connectionStore.upsert(
          payload.companyName,
          payload.config
        ).id;
        persisted = true;
      ...
      if (!id) {
        return {
          ok: false,
          error: 'upsert-connection: neither config nor connectionId provided',
        };
      }
      if (
        payload.companyName !== undefined ||
        payload.openCount !== undefined
      ) {
        persisted =
          this.connectionStore.setMeta(id, {
      ...
      if (!persisted) {
        return {
          ok: false,
          error: `upsert-connection: unknown connection ${id}`,
        };
      }
      this.connectionStore.setLastSelected(id);
      return { ok: true, id };
    } catch (err) {
      return { ok: false, error: (err as Error).message };
    }
  ...
  async dbCall(
    ...
    return getErrorHandledResponse(async () => {
      ...
      switch (method) {
        ...
        case 'get': {
          const [schemaName, name, fields] = args as [
            string,
            string,
        ...
        case 'getAll': {
          const [schemaName, options] = args as [
            string,
        ...
        case 'getSingleValues': {
          return db.getSingleValues(
            ...(args as ({ fieldname: string; parent?: string } | string)[])
          );
        }
        case 'rename': {
          const [schemaName, oldName, newName] = args as [
            string,
            string,
            string,
          ];
  ...
  async selectFile(
    ...
    const { filePaths, canceled } = await this.deps.dialog.showOpenDialog(
      window!,
  ...
  async getLanguageMap(code: string) {
    const obj = { languageMap: {}, success: true, message: '' } as {
      languageMap: unknown;
      success: boolean;
      message: string;
    };
  ...
  getCreds() {
    ...
    const { getUrlAndTokenString: getCredsFn } =
      require('../contactMothership') as typeof import('../contactMothership');
    return getCredsFn();
  }
  ...
  async checkForUpdates(
    ...
    try {
      const updater =
        this.deps.autoUpdater ?? (await import('electron-updater')).autoUpdater;
      await updater.checkForUpdates();
    } catch (error) {
      // Lazy to avoid pulling electron-store at module load
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      const { isExpectedUpdateError: isExpected } =
  ...
  register(): void {
    ...
    const withSender =
      <T extends unknown[]>(
        fn: (event: Electron.IpcMainInvokeEvent, ...args: T) => Promise<unknown>
      ) =>
    ...
    ipc.handle(
      ...
      withSender(async (_e, dbPathOrConfig: string | MariaDBConfig) => {
        const cfg =
          typeof dbPathOrConfig === 'string'
            ? this.peekConfig(dbPathOrConfig)
            : dbPathOrConfig;
    ...
    ipc.handle(
      ...
      withSender(async (_e, config: MariaDBConfig) =>
        this.checkDbExists(config)
      )
    );
    ipc.handle(
    ...
    ipc.handle(
      ...
      withSender(async (_e, dbPath: string, countryCode: string) =>
        this.createNewDatabase(dbPath, countryCode)
      )
    );
    ipc.handle(
    ...
    ipc.handle(
      IPC_ACTIONS.DB_CALL,
      withSender(async (_e, method: DatabaseMethod, ...args: unknown[]) =>
        this.dbCall(method, ...args)
      )
    );
    ipc.handle(
      IPC_ACTIONS.DB_SCHEMA,
      withSender(async () => this.dbSchema())
    );
    ipc.handle(
      IPC_ACTIONS.GET_DB_LIST,
      withSender(async () => this.getDbList())
    );
    ipc.handle(
      ...
      withSender(
        ...
        ) => this.upsertConnection(payload)
      )
    );

    // InstallerOps
    ipc.handle(
      IPC_ACTIONS.GET_LAN_IP,
      withSender(async () => this.getLanIp())
    );
    ipc.handle(
      IPC_ACTIONS.PING_MARIA_DB,
      withSender(async (_e, opts: PingOptions) => this.pingMariaDB(opts))
    );
    ipc.handle(
      IPC_ACTIONS.INSTALL_MARIA_DB,
      withSender(async (e, opts: HostProvisionConfig) =>
        this.provisionMariaDB(
          opts,
          (e as unknown as { sender: Electron.WebContents }).sender ??
            w()?.webContents ??
            null
        )
      )
    ...
    ipc.handle(
      ...
      withSender(async (event, data: string, savePath: string) =>
        this.saveData(data, savePath, event)
      )
    );
    ipc.handle(
      IPC_ACTIONS.DELETE_FILE,
      withSender(async (event, filePath: string) =>
        this.deleteFile(filePath, event)
      )
    );
    ipc.handle(
      IPC_ACTIONS.SELECT_FILE,
      withSender(async (_e, options: SelectFileOptions) =>
        this.selectFile(options, w())
      )
    );
    ipc.handle(
      IPC_ACTIONS.GET_OPEN_FILEPATH,
      withSender(async (_e, options: OpenDialogOptions) =>
        this.getOpenFilePath(options, w())
      )
    );
    ipc.handle(
      IPC_ACTIONS.GET_SAVE_FILEPATH,
      withSender(async (_e, options: SaveDialogOptions) =>
        this.getSaveFilePath(options, w())
      )
    );
    ipc.handle(
      IPC_ACTIONS.GET_DIALOG_RESPONSE,
      withSender(async (_e, options: MessageBoxOptions) =>
        this.getDialogResponse(
          options,
          w(),
          this.deps.windowProvider.isDevelopment,
          this.deps.windowProvider.isLinux,
          this.deps.windowProvider.icon
        )
      )
    );
    ipc.handle(
      ...
      withSender(
        async (
          ...
          width: number,
          height: number
        ) => this.saveHtmlAsPdf(html, savePath, width, height, this.deps.app)
      )
    );
    ipc.handle(
      IPC_ACTIONS.PRINT_HTML_DOCUMENT,
      withSender(async (_e, html: string, width: number, height: number) =>
        this.printHtmlDocument(html, width, height, this.deps.app)
      )
    );
    ...
    ipc.handle(
      ...
      withSender(
        async (_e, { title, content }: { title: string; content: string }) =>
          this.showError(title, content)
      )
    );
    ipc.handle(
      IPC_ACTIONS.SEND_ERROR,
      withSender(async (_e, bodyJson: string) =>
        this.sendError(bodyJson, this.deps.windowProvider as unknown)
      )
    );
    ipc.handle(
      ...
      withSender(
        ...
        async (
          ...
          endpoint: string,
          options: NodeFetchRequestInit | undefined
        ) => this.sendAPIRequest(endpoint, options, event)
      )
    );
    ipc.handle(
      ...
      withSender(async () =>
        this.checkForUpdates(
          this.deps.windowProvider.isDevelopment,
          this.deps.windowProvider.checkedForUpdate,
          () => {
            this.deps.windowProvider.checkedForUpdate = true;
          }
        )
      )
    );
    ipc.handle(
      ...
      withSender(async (_e, code: string) => this.getLanguageMap(code))
    );
    ipc.handle(
      IPC_ACTIONS.GET_CREDS,
      withSender(
        // eslint-disable-next-line @typescript-eslint/require-await
        async () => this.getCreds()
      )
    ...
    ipc.handle(
      ...
      withSender(async (_e, posPrintWidth?: number) =>
        this.getTemplates(posPrintWidth)
      )
    );
    ipc.handle(
      IPC_ACTIONS.INIT_SHEDULER,
      withSender(async (_e, interval: string) =>
        this.initScheduler(this.deps.windowProvider as unknown, interval)
      )
    );
  }
}
...
export function createProdRouter(main: import('../bootstrap').Main): IpcRouter {
  const winProvider: WindowProvider = {
    ...
    get isDevelopment() {
      return main.isDevelopment;
    },
    get isLinux() {
      return main.isLinux;
    },
    get checkedForUpdate() {
      return main.checkedForUpdate;
    },
    set checkedForUpdate(v: boolean) {
      main.checkedForUpdate = v;
    },
    get icon() {
      return main.icon;
    },
  };
  const { app: electronApp, dialog: electronDialog } =
    require('electron') as typeof import('electron');
  // Lazy to avoid pulling the mariadb driver into unit tests at module load
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { default: MainDatabase } =
    require('../../backend/database/core') as typeof import('../../backend/database/core');
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { ElectronStoreConnectionStore } =
    require('./connectionStore') as typeof import('./connectionStore');
